      ...
      <div class="row">
        ...
        <div class="form-login d-flex flex-row justify-content-center align-items-center my-flex-container">
             <div class="col-8 my-flex-item">
               <p class="h4">Sign In to Sails VMS Web</p>
               <p>Enter your details below.</p>
               <form id="login" method="post" action="{{route('login_sl')}}">
                <input type="hidden" name="_token" id="_token" value="{{ csrf_token() }}">
                 <div class="form-group">
                   <label for="exampleInputEmail1" class="h6">Username</label>
                   <input type="text" class="form-control" name="username" id="username" aria-describedby="emailHelp" placeholder="Username" autofocus="autofocus">
                 ...
                 <div class="form-group">
                   <label for="exampleInputPassword1" class="h6">Terminal
                   </label>
                   <span style="right: 0; position: absolute; font-size: 12px; font-weight: 400;" class="pr-3"></span>
                   <select class="form-control" name="terminal" id="terminal">
                    <option value="">Pilih Terminal</option>
                    @foreach ($terminal as $value)
                      <option value="{{$value->ID_TERMINAL}}">{{$value->TERMINAL_NAME}}</option>
                    @endforeach
                  </select>
